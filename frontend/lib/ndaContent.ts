import type { CoverPageData, NdaFormData } from "./types";

export const NDA_DEFAULTS = {
  purpose:
...
export function buildCoverPageData(form: NdaFormData): CoverPageData {
  return {
    purpose: NDA_DEFAULTS.purpose,
    effectiveDate: formatDate(form.effectiveDate),
    mndaTerm: `Expires ${NDA_DEFAULTS.termYears} year(s) from Effective Date.`,
    confidentialityTerm: `${NDA_DEFAULTS.termYears} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.`,
    governingLaw: form.governingLaw || "[Fill in state]",
    jurisdiction: form.jurisdiction || "[Fill in city or county and state]",
    modifications: NDA_DEFAULTS.modifications,
    party1: {
      name: form.party1Name || "[Party 1 name]",
      company: form.party1Company || "[Party 1 company]",
    },
    party2: {
      name: form.party2Name || "[Party 2 name]",
      company: form.party2Company || "[Party 2 company]",
    },
  };
}
...
export const STANDARD_TERMS_SECTIONS: StandardTermsSection[] = [
  {
